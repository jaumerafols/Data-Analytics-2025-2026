...
CREATE TABLE products (
    ...
    price DECIMAL(10,2), -- fem la correcció
    colour VARCHAR(20),
    weight DECIMAL(10,2), -- fem la correcció
...
CREATE TABLE IF NOT EXISTS users (
    ...
    phone VARCHAR(150),
    email VARCHAR(150),
    birth_date DATE, -- fem correcció hi havia VARCHAR(50)
    country VARCHAR(150),
    city VARCHAR(150),
...
CREATE TABLE IF NOT EXISTS credit_cards  (
    ...
    iban VARCHAR(34),
    pan VARCHAR(20), 
    pin CHAR(4), -- fem correcció hi havia VARCHAR
    cvv CHAR(3), -- fem correcció hi havia VARCHAR
    track1 VARCHAR(255),
    track2 VARCHAR(255),
    expiring_date DATE -- fem correcció hi havia VARCHAR(20)
);
...
LINES TERMINATED BY '\n'
IGNORE 1 ROWS
(id_credit_card, user_id, iban, pan, pin, cvv, track1, track2, @v_expiring_date)
SET expiring_date = STR_TO_DATE(@v_expiring_date, '%m/%d/%y'); -- afegim a correcció %m %d i %y ('y' en minúscula perqué en taula ve amb dos dígits)
...
JOIN transactions t ON tp.id_transaction = t.id_transaction
WHERE t.declined = 0
